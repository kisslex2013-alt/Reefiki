...
Use this skill when the user asks to run the migrated source command `connect`.
...
5. **Зафиксировать git-bridge contract в кодовом проекте.**
   ...
   - Канон: `_wiki/` — bridge/junction для чтения, поиска и локального контекста, не часть git-истории кодового проекта.
   - Прямые durable writes через `_wiki/**` запрещены: они обходят scoped write gates и могут конфликтовать с параллельными тредами. Для записи использовать REEFIKI write path (`wiki-write`, `harvest-commit`, `/process` или `/harvest`) с явным target project.
...
8. **Добавить wiki-инструкции в AGENTS.md кодового проекта** (если файл существует).
   ...
   **Git contract:**
   - `_wiki/` — bridge/junction для чтения, поиска и локального контекста, не часть git-истории этого кодового проекта.
   - durable knowledge changes писать только через REEFIKI write path: `wiki-write`, `harvest-commit`, `/process` или `/harvest` с явным target project.
   - не редактировать `_wiki/**` напрямую через файловый write: прямые записи обходят secret-scan/scope/backlink gates и могут конфликтовать с параллельными тредами.
   ...
   **Триггеры:**
   - «Сохрани в вики» / «запомни» → выполнить REEFIKI write path (`wiki-write`, `/process` или `/harvest`), не прямой write в `_wiki/`
...
10. **Финал.** Сказать:
   > Готово. Проект подключён к вики. Теперь при работе в `<путь>` агент видит `_wiki/` для чтения и может сохранять знания через безопасный REEFIKI write path. Просто скажи «запомни это» или «сохрани решение» — и оно попадёт в вики.
