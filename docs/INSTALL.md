...
Если у тебя уже есть Markdown/Obsidian/Logseq заметки, импортируй их после `doctor`, но до `/process`: `reefiki --project <project> import <notes-path> --from markdown --format json`. Это только заполняет `inbox/`; durable wiki-страницы по-прежнему создаются через разбор копилки.
...
Maintainers should publish Python package artifacts through the manual `Publish Python Package` GitHub Actions workflow. The workflow is tokenless: it builds and tests distributions first, then uses PyPI Trusted Publishing only in the selected publish job.

Before the first upload, configure trusted publishers on both TestPyPI and PyPI. For the first `reefiki` release, use the account-level publishing page to add a pending publisher for:

- project/package: `reefiki`;
- repository: `kisslex2013-alt/Reefiki`;
- workflow: `.github/workflows/publish-python.yml`;
- environments: `testpypi` for TestPyPI and `pypi` for PyPI.

Run `target=build-only` first to prove the manual workflow can build and check artifacts without contacting a package index. Then run `target=testpypi` and install-smoke from TestPyPI before publishing to PyPI. The `target=pypi` path is intentionally guarded to run only from `refs/tags/v*` and should use the protected `pypi` environment. Do not add long-lived PyPI/TestPyPI tokens for this release path.
