...
def _is_busy_lock_error(exc: OSError, lock_path: Path) -> bool:
    ...
    # PermissionError while another process still owns the path. The file can
    # also disappear before the exists() probe, so retry lock-path permission
    # errors until the normal timeout instead of surfacing a flaky failure.
