...
def test_init_cli_creates_valid_indexed_project(tmp_path: Path, capsys) -> None:
    ...
    assert "projects/first-run/.gitignore" in payload["created_paths"]
    ...
    assert ".reefiki/" in (project / ".gitignore").read_text(encoding="utf-8")
...
def test_init_project_gitignore_keeps_local_index_out_of_git(tmp_path: Path) -> None:
    ...
    staged = run_git(workspace, ["diff", "--cached", "--name-only"]).stdout

    assert "projects/first-run/.gitignore" in staged
    assert "projects/first-run/.reefiki/index.sqlite" not in staged
    assert "projects/first-run/.reefiki/index.lock" not in staged
